...
//--- Write DLC field, FDF, BRS, RTR, IDE bits
  ...
  // Serial.print("\nAPPENDING TX CONTROLLER FIFO\n");
  // Serial.print("Flags: ");
  // Serial.println(flags, BIN);
  // Serial.print("IDF: ");
  // Serial.println(idf, BIN);
  if (inMessage.ext) {
  ...
  case CANFDMessage::CANFD_NO_BIT_RATE_SWITCH :
    Serial.println("CANFD_NO_BIT_RATE_SWITCH");
    flags |= 1 << 7 ; // Set FDF bit
    break ;
  case CANFDMessage::CANFD_WITH_BIT_RATE_SWITCH :
    
    // The ACAN2517FD library does not support CANMessage without FD. This fixes the control bit issue.
    // flags |= 1 << 7 ; // Set FDF bit
    // if (mHasDataBitRate) {
    //   Serial.println("Has Data Bit Rate");
